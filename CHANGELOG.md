...
- **/bot content/save (HAR)**: inspect dumps `properties.settings` plus sibling keys the back-office saved (`viewMode`, `perLine`, `thumbnailPosition`, `uncompressedThumbnail`, `directory` as an array, `imageFormat`, html `content`). `class` is a single token; `widgetClass` is a comma list. Live CSS stays **unknown** (no quote that the front prefixes `.widget--`). Footer link is `unset` only when that field is absent. Parent prints template **row uuid** / **cell uuid**. Empty `properties.style` omitted unless `verbose=true`. Writes remain `content/save`; widget `/blocks` is a post-save render (`ownerResourceInfo` + `widgetComponent`). Default inspect shows one component tree; `verbose=true` prints both.
