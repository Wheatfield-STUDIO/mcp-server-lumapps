...
TOOL_SCHEMA = {
    "name": TOOL_NAME,
    "description": (
        "Update the global CSS of a LumApps site via the style API. Use this tool for design system or branding: primary color, shadows, border-radius, button style, site background, etc. After search_site you can call this tool directly for :root variables. For widget or page CSS you MUST run inspect_lumapps_element first, then inspect_widget_render (cssClass skin), then inspect_front_html for deep widget CSS (.lumx-*, title/span from pasted outerHTML). Use only selectors those tools listed. skin: .{cssClass} → .widget--{cssClass} (/blocks token; CSS uses the prefix; proven content-list / directory). Do not invent .lumx-* from /blocks. properties.widgetClass is not the skin hook. Before calling, you MUST present the modification to the user and wait for their explicit 'Yes' or 'Confirm'. Never apply changes silently. "
        "CSS variables: For design system changes you MUST use LumApps CSS variables from the MCP resource lumapps-css-variables (uri: lumapps://lumapps-mcp-server/css-variables). Set variables in :root (e.g. --lumx-color-primary-N, --lumx-app-background, --lumx-app-header-box-shadow, --lumx-button-border-radius, --lumx-button-emphasis-high-state-*-border-width, --lumx-text-field-state-default-theme-light-input-border-color). Do NOT use variables that are not in that resource: no --lumx-shadow-1/2/3/4/5, no --lumx-color-primary-500/600/700. That resource lists variables, not DOM classes — missing .lumx-button there does not mean the class is fake. Classes listed by a prior inspect_front_html (e.g. .lumx-button when present in pasted outerHTML) are legitimate selectors. Do not invent class names. Do NOT use raw selectors like button, [class*='button'], [class*='primary'] for theme-wide changes; use variables. Only if no variable exists (e.g. button text-transform), use a minimal override with inspect_front_html selectors. "
        "When updating CSS: "
